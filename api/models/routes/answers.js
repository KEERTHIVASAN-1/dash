...
router.post('/', authenticateToken, validateAnswer, async (req, res) => {
  try {
    ...
    // Mark related question as resolved (answered)
    await Question.findByIdAndUpdate(answer.question, { isResolved: true });

    // Load question to provide context in notification
    const question = await Question.findById(answer.question).select('title');

    // Send notification to teachers about the new answer
    await notifyTeachersNewAnswer(answer, question);
...
// Get answers for a user (with pagination) and likes count summary
router.get('/user/:userId', validateObjectId('userId'), validatePagination, async (req, res) => {
  try {
    ...
    // Total likes across this user's answers
    const likesCountAgg = await Answer.aggregate([
      { $match: { author: require('mongoose').Types.ObjectId(req.params.userId) } },
      { $project: { likesSize: { $size: '$likes' } } },
      { $group: { _id: null, totalLikes: { $sum: '$likesSize' } } }
    ]);
    const totalLikes = likesCountAgg[0]?.totalLikes || 0;
