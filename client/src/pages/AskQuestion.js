...
const AskQuestion = () => {
  ...
  const createQuestionMutation = useMutation(questionsAPI.create, {
    ...
    onError: (error) => {
      ...
      // Map express-validator errors array to field object, if present
      if (Array.isArray(serverErrors) && serverErrors.length > 0) {
        const fieldErrors = serverErrors.reduce((acc, err) => {
      ...
      toast.error(serverMessage || 'Failed to post question');
    }
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.content.trim().length < 20) newErrors.content = 'Content must be at least 20 characters';
