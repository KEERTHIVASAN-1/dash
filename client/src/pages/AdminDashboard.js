...
const AdminDashboard = () => {
  ...
  const { data: questions, isLoading: questionsLoading } = useQuery(
    ['admin-questions', { page: 1, limit: 20 }],
    () => adminAPI.getQuestions({ page: 1, limit: 20 }),
    {
      enabled: isAdmin() || isTeacher(),
    }
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600">Manage users, questions, and platform statistics</p>
      </div>
      ...
      {/* Content */}
      <Routes>
        <Route path="/" element={<Overview stats={stats} statsLoading={statsLoading} />} />
        <Route 
          path="/users" 
          element={
            <UsersManagement 
...
// Overview Component
const Overview = ({ stats, statsLoading }) => {
  if (statsLoading) {
    return (
          ...
          <div className="flex items-center">
            ...
            <div className="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd className="text-lg font-medium text-gray-900">{stats?.stats?.users?.total || 0}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd className="text-lg font-medium text-gray-900">{stats?.stats?.content?.questions || 0}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd className="text-lg font-medium text-gray-900">{stats?.stats?.content?.resolved || 0}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd className="text-lg font-medium text-gray-900">{stats?.stats?.users?.active || 0}</dd>
              </dl>
            </div>
          </div>
        </div>
      ...
      <div className="card">
        ...
        <div className="space-y-3">
          {stats?.stats?.recentQuestions?.map((question) => (
            <div key={question._id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
...
const UsersManagement = ({ users, usersLoading, onRoleChange, onStatusToggle }) => {
  if (usersLoading) {
    return (
      <div className="card">
